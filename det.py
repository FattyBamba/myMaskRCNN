...
class DetDataset(utils.Dataset):
    ...
    def load_dataset_images(self, dataset_dir, subset, class_names):
        ...
        for i, name in enumerate(class_names):
            ...
            if name != 'BG':
                print('Adding class {:3}:{}'.format(index, name))
                self.add_class('building', index, name)
...
if __name__ == "__main__":
    config = DetConfig('building', ['building'])
    dataset_train, dataset_val = create_datasets('./images/ActiOn/train', config)
